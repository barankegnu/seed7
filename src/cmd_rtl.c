...
#if INTTYPE_SIZE >= INT_SIZE
#define CONFIG_VALUE_BUFFER_SIZE INTTYPE_DECIMAL_SIZE + NULL_TERMINATION_LEN
#else
#define CONFIG_VALUE_BUFFER_SIZE INT_DECIMAL_SIZE + NULL_TERMINATION_LEN
#endif
...
#elif defined DEFINE_WIN_SYMLINK && defined HAS_DEVICE_IO_CONTROL
...
striType cmdConfigValue (const const_striType name)
  ...
  /* cmdConfigValue */
    ...
    if (unlikely(name->size > MAX_CSTRI_BUFFER_LEN)) {
      ...
      opt = "FALSE";
#endif
    } else if (strcmp(opt_name, "USE_START_MAIN") == 0) {
#ifdef USE_START_MAIN
      opt = "TRUE";
#else
      opt = "FALSE";
#endif
